...
describe("Performance Tests", () => {
  ...
  describe("Response Time Performance", () => {
    test("should register user within acceptable time (<500ms)", async () => {
      ...
      console.log(`Registration time: ${duration}ms`);
    });

    test("should login user within acceptable time (<300ms)", async () => {
      // Create user first
      await request(app).post("/api/auth/register").send({
        email: "login-perf@test.com",
        password: "Password123",
        firstName: "Login",
        lastName: "Perf",
        role: "driver",
    ...
    test("should retrieve chat messages within acceptable time (<200ms)", async () => {
      const roomId = "perf-room";

      // Create some messages
      await ChatMessage.insertMany([
        { roomId, sender: "user1", message: "Message 1" },
        { roomId, sender: "user2", message: "Message 2" },
        { roomId, sender: "user3", message: "Message 3" },
      ]);
      ...
      console.log(`Chat retrieval time: ${duration}ms`);
    });
  });
  ...
  describe("Concurrent User Performance", () => {
    ...
    test("should handle 100 concurrent login attempts", async () => {
      // Create users first
      const userCount = 100;
      ...
      console.log(
        `100 concurrent logins: ${duration}ms (${(duration / 100).toFixed(2)}ms avg)`
      );
    });
  ...
  describe("Database Performance Under Load", () => {
    test("should handle large message history efficiently", async () => {
      const roomId = "large-history-room";
      ...
      expect(duration).toBeLessThan(1000); // Should retrieve 1000 messages in <1s
    ...
    test("should maintain performance with multiple rooms", async () => {
      ...
      const response = await request(app)
        .get("/api/chat/multi-room-10")
        .expect(200);

      const endTime = Date.now();
      const duration = endTime - startTime;

      expect(response.body).toHaveLength(messagesPerRoom);
      expect(duration).toBeLessThan(500);

      console.log(
        `Retrieved room from ${roomCount} rooms with ${roomCount * messagesPerRoom} total messages in ${duration}ms`
      );
    });
  });
  ...
  describe("Memory Performance", () => {
    test("should not leak memory with repeated operations", async () => {
      ...
      console.log(
        `Memory increase after ${iterations} operations: ${memoryIncreaseMB}MB`
      );
...
describe("Load Tests", () => {
  ...
  beforeAll(async () => {
    ...
    ioServer.on("connection", (socket) => {
      ...
      socket.on("send_message", async (data) => {
        ...
        ioServer
          .to(roomId)
          .emit("receive_message", { sender, message, timestamp: new Date() });
      });
    });
  ...
  describe("Socket.IO Load Testing", () => {
    ...
    test("should handle multiple messages from multiple users", async () => {
      const userCount = 10;
      ...
      for (let i = 0; i < userCount; i++) {
        for (let j = 0; j < messagesPerUser; j++) {
          promises.push(
            new Promise((resolve) => {
              clients[i].emit("send_message", {
                roomId,
                sender: `user${i}`,
                message: `Message ${j} from user ${i}`,
              });
              clients[i].once("receive_message", resolve);
            })
          );
        }
      }
      ...
      console.log(
        `${totalMessages} messages from ${userCount} users in ${duration}ms (${(duration / totalMessages).toFixed(2)}ms avg)`
      );

      // Verify all messages saved
      const savedMessages = await ChatMessage.find({ roomId });
      expect(savedMessages).toHaveLength(totalMessages);
